...
public:
  ...
  template <typename T>
  ArrayT<T>& ArrayT<T>::operator*( ArrayT<T>& A ) {
  ...
  // ----------- 1D*1D ---------------
      ...
      if( dims == 1 && A.dims == 1 ) {
          if( slot1 != A.slot1 ) {
              printf("Dimensions are not the same size!\n");
              exit(0);
          }

          vector<T> sol;
      T sum = 0;
          printf( "sol.size() before assignment = %lu \n", sol.size() );
          for( int i = 0; i < slot1; i++ )
              sum += ( (*this)(i)*A(i) );

      sol.push_back( sum );

          printf("sol.size() after assignment = %lu \n", sol.size() );
          return *(new ArrayNd<T>( sol, 1 ));
      }

  // ----------- 2D*1D ---------------

    else if( dims == 2 && A.dims == 1 ) {
          if( slot2 != A.slot1 ) {
              printf("Dimensions are not the same size!\n");
              exit(0);
          }
      ...
      for( int i = 0; i < slot1; i++ ) {
        ...
        sol.push_back( sum );
          ...
          printf("sol.size() after assignment = %lu \n", sol.size() );
          return *(new ArrayNd<T>( sol, 1, slot1 ));
      }
       
  // ----------- 3D*1D ---------------

    else if( dims == 3 && A.dims == 1 ) {
          if( slot3 != A.slot1 ) {
              printf( "Dimensions are not the same size!\n" );
              exit(0);
          }
      ...
      for( int j = 0; j < slot2; j++ ) {
        for( int i = 0; i < slot1; i++ ) {
          ...
          printf( "sol.size() after assignment = %lu \n", sol.size() );
          return *(new ArrayNd<T>( sol, 2, slot1, slot2 ));
      }

  // ----------- 2D*2D ---------------

    else if( dims == 2 && A.dims == 2 ) {
          if( slot2 != A.slot1 || slot1 != A.slot2 ) {
              printf( "Dimensions are not the same size!\n" );
              exit(0);
          }
  ...
  // ----------- 3D*2D ---------------
    ...
    else if( dims == 3 && A.dims == 2 ) {
          if( slot3 != A.slot1 || slot2 != A.slot2 ) {
              printf( "Dimensions are not the same size!\n" );
              exit(0);
          }
  ...
  // ----------- 3D*3D ---------------
    ...
    else if( dims == 3 && A.dims == 3 ) {
          if( slot3 != A.slot1 || slot2 != A.slot2 || slot1 != A.slot3 ) {
              printf( "Dimensions are not the same size!\n" );
              exit(0);
          }
    ...
    virtual void init(T* array, int n1_, int n2_, int n3_=1, int n1m_=0, int n2m_=0, int n3m_=0) {
    ...
    inline T& get(const int i, const int j, const int k=0) const {
        return data[(i-n1m)+np1*((j-n2m)+np2*(k-n3m))];
    }
...
ArrayT<T>& ArrayT<T>::operator+(ArrayT<T>& a)
{
		printf("inside operator+ non-const\n");
		const int* dimsa = a.getDims();
		if (dimsa[0] != dims[0] || dims[1] != dims[1] || dims[2] != dims[2]) {
			printf("operator+: inconsistent array sizes\n");
		}
		int nbPts = a.getSize();
...
{ 
//	printf("in operator* double*array\n");
	const int* dims = a.getDims();
	//printf("operator, dims= %d, %d, %d\n", dims[0], dims[1], dims[2]);
	int nbPts = a.getSize();
	T* ap = a.getDataPtr();
	ArrayT<T>& r = *(new ArrayT<T>(dims));
	T* rp = r.getDataPtr();
	...
	const int* dims1 = r.getDims();
	printf("dims(r) = %d, %d\n", dims1[0], dims1[1]);
	return r;
}

template <class T>
ArrayT<T>& operator*(ArrayT<T>& a, double b)   //  T + array
{
//	printf("in operator* array*double\n");
	const int* dims = a.getDims();
	//printf("operator, dims= %d, %d, %d\n", dims[0], dims[1], dims[2]);
	int nbPts = a.getSize();
	T* ap = a.getDataPtr();
